pipeline {
    ...
    stages {
        ...
        stage('Login to Docker Hub') {
            steps {
                withCredentials([string(credentialsId: 'duckerhubpassword', variable: 'docker-pass')]) {
                    script {
                        if (isUnix()) {
                            sh "docker login -u ndissanayake -p ${docker-pass}"
                        } else {
                            bat "docker login -u ndissanayake -p ${docker-pass}"
                        }
                    }
                }
            }
